...
$(document).ready(function() {
    $('#pay_now').click(function(e) {
        ...
        var locale = "{{ app()->getLocale() }}"; // Assuming you're using Blade to set the locale dynamically
        var formData = new FormData($('#kt_subscriptions_create_new')[0]); // Get form data

        $.ajax({
            url: `/${locale}/subscribe`, // Adjust URL to your needs
            type: 'POST',
            data: formData,
            contentType: false, // Necessary for file upload
            processData: false, // Necessary for file upload
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Ensure you have a meta tag for CSRF token in your head tag
            },
