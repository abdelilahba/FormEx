...
<script>
    ...
    // Check if the item.id exists, indicating it's not a placeholder
    if (!item.id) {
        return item.text;
    }

    // Create a new span element to hold the formatted item
    var span = document.createElement('span');
    
    // Dynamically retrieve the image URL from a data attribute, falling back to a default if necessary
    var imgUrl = item.element.getAttribute('data-img-url') || 'default_image_path_here';

    // Build the template with the image and text
    var template = '<img src="' + imgUrl + '" class="rounded-circle h-20px me-2" alt="image"/>' + item.text;

    // Set the inner HTML of the span to the template
