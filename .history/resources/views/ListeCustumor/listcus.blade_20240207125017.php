...
function openEditModal(customerId) {
    currentCustomerId = customerId; // Set the current editing ID
    $('#customerForm')[0].reset(); // Reset form (in case it was previously used for add)
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // Continue setting values for all fields...

            $('#kt_modal_add_customer .modal-title').text('Edit Customer'); // Update modal title
            $('#kt_modal_add_customer').modal('show'); // Show modal
        },
        error: function(xhr, status, error) {
...
$('#customerForm').submit(function(e) {
    ...
    var formAction = currentCustomerId ? `/customers/${currentCustomerId}/update` : "{{ route('customers.store') }}";
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            Swal.fire('Error!', 'There was an issue with the customer operation. Please try again.', 'error');
        }
    });
});
