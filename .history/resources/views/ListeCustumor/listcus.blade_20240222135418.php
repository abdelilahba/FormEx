...
<style>
    ...
    .badge {
        ...
        /* For aligning with other action buttons */
    }
...
<script>
  $(document).on('click', '.validate-btn', function() {
    ...
    var customerId = $button.data('id'); // Customer ID

    Swal.fire({
        title: 'Are you sure?',
        text: "You want to validate this customer!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                data: {
                    ...
                    validate: 1 // Setting validate to 1
                },
                success: function(response) {
                    Swal.fire('Validated!', 'The customer has been validated.', 'success');
                    $button.hide(); // Hide the "Validate" button
                    
                    // Optionally, you can update the DataTable row to reflect the new validation state without reloading the page
                    // This could involve changing the row color, updating a badge, etc.
                },
                error: function(xhr, status, error) {
                    Swal.fire('Error!', 'An error occurred during the validation process.', 'error');
                }
            });
        }
    });
