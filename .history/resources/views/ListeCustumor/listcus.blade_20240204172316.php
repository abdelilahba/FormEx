...
<script>
    // After your DataTables initialization code
$(document).on('click', '.activate-btn', function() {
    var customerId = $(this).data('id'); // Get the ID of the customer

    $.ajax({
        url: '/path-to-your-endpoint', // The endpoint on your server that handles activation
        type: 'POST',
        data: {
            id: customerId,
            state: 1, // Assuming 1 is the state for "Active"
            _token: $('meta[name="csrf-token"]').attr('content') // CSRF token, if needed
        },
        success: function(response) {
            // Handle success. For example, reload the DataTable to show the updated state
