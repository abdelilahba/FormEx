...
<form action="{{ route('language.change') }}" method="POST" class="d-inline">
            @csrf
            <select class="form-select form-control" onchange="this.form.submit()"  name="country" id="kt_docs_select2_country">
               <option value="AF" {{ App::getLocale() == 'ar' ? ' selected' : '' }}  data-kt-select2-country="assets/media/flags/morocco.svg">العربية</option>
               <option value="en"{{ App::getLocale() == 'en' ? ' selected' : '' }} data-kt-select2-country="assets/media/flags/english.svg" >English</option>
               <option value="AX" {{ App::getLocale() == 'fr' ? ' selected' : '' }} data-kt-select2-country="assets/media/flags/france.svg">Français</option>
            </select>
