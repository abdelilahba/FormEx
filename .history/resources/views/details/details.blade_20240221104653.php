                    ...
                    <form class="form fv-plugins-bootstrap5 fv-plugins-framework" id="customerForm" method="POST" action="{{ route('profile.complete', ['locale' => app()->getLocale()]) }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="d-flex justify-content-around mt-4">
                                ...
                                <a href="{{ url(app()->getLocale() . '/subsciption/' . $customer->random_number) }}" id="request_demo">{{ __('messages.request_demo') }}</a>

                                <!-- Subscription button -->
                                <a href="#detailsLinkPage" class="btn btn-success">{{ __('messages.subscribe') }}</a>
                            </div>
                    </form>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.request-pack').forEach(button => {
        button.addEventListener('click', function() {
            ...
            const productType = encodeURIComponent(this.getAttribute('data-product-type'));
            const usersIncluded = this.getAttribute('data-users-included');
            const storage = encodeURIComponent(this.getAttribute('data-storage'));
            ...
            const locale = "{{ app()->getLocale() }}"; // Assuming you have Laravel's localization set up
