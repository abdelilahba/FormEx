                    ...
                    <form class="form fv-plugins-bootstrap5 fv-plugins-framework" id="customerForm" method="POST" action="{{ route('profile.complete', ['locale' => app()->getLocale()]) }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="d-block text-center mt-4">
                                <!-- Demander un demo button -->
                                <button class="btn btn-success text-white" type="submit" id="request_demo">{{ __('messages.request_demo1') }}</a>
...
<script>
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (successMessage) {
            Swal.fire({
                icon: 'success',
                title: currentLocale === 'fr' ? 'Succès!' : 'تم ارسال بنجاح سيتم تفعيل طلبكم لتصلكم النسخة!',
                text: successMessage,
            });
...
<script>
    var customerRandomNumber = "{{ $customer->random_number }}";
    document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.request-pack').forEach(button => {
        button.addEventListener('click', function(event) {
            event.preventDefault(); // Prevent the default form submission if necessary

            const form = document.getElementById('customerForm'); // Reference to your form
            const formData = new FormData(form); // Gather all form data
            
            // Extract additional data attributes from the button
            formData.append('price', this.getAttribute('data-price'));
            formData.append('productType', this.getAttribute('data-product-type'));
            formData.append('usersIncluded', this.getAttribute('data-users-included'));
            formData.append('storage', this.getAttribute('data-storage'));

            // Construct query parameters from form data
            const queryParams = new URLSearchParams();
            for (const [key, value] of formData.entries()) {
                if (key !== 'templates[]') { // Exclude file inputs from query parameters
                    queryParams.append(key, value);
                }
            }

            const locale = form.querySelector('input[name="locale"]').value || "{{ app()->getLocale() }}";
            const redirectUrl = `/${locale}/subscription/${customerRandomNumber}?${queryParams}`;
