                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if(row.state === "0") { // Show activate button only for pending (state 0) customers
                            buttonHTML= `<button class="btn btn-sm btn-outline-success activate-btn"  data-id="${row.id}" style="text-aling:center" title="Activate Customer"><i class="fa fa-check-circle"></i> Activate</button>`;
                        } else  {
                            buttonHTML =`<button class="btn btn-sm btn-outline-danger toggle-activation-btn" data-id="${row.id}" data-action="deactivate" style="text-align:center" title="Deactivate Customer"><i class="fa fa-ban"></i> Deactivate</button>`;; // No button for active or refused states
                        }
                        return buttonHTML;
                    }
                },
