            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (row.invoice && row.invoice.length > 0) {
                            var latestInvoice = row.invoice[row.invoice.length - 1]; // Assuming the last invoice is the latest
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        var badgeText = '';
                        switch(data) {
                            case "0": badgeClass = 'badge badge-info'; badgeText = 'Pending'; break;
                            case "1": badgeClass = 'badge badge-success'; badgeText = 'Active'; break;
                            case "2": badgeClass = 'badge badge-danger"'; badgeText = 'Refused'; break;
                            default: badgeClass = 'badge badge-dark'; badgeText = 'Unknown';
                        }
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        var buttonHTML = '';
                        if(row.state === "0") {
                            // Customer is pending, show activate button
                            buttonHTML = `<button class="btn btn-sm btn-outline-success toggle-activation-btn" data-id="${row.id}" data-action="activate" style="text-align:center" title="Activate Customer"><i class="fa fa-check-circle"></i> Activate</button>`;
                        } else if(row.state === "1") {
                            // Customer is active, show deactivate button
                            buttonHTML = `<button class="btn btn-sm btn-outline-danger toggle-activation-btn" data-id="${row.id}" data-action="deactivate" style="text-align:center" title="Deactivate Customer"><i class="fa fa-ban"></i> Deactivate</button>`;
                        }
                        return buttonHTML;
                    }
                },
