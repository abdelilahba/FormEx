...
public function activate(Request $request)
{
    ...
    if ($customer) {
        ...
        // Generate the link
        $companyShort = strtolower(substr($customer->company_name, 0, 5)); // Get first 5 chars of company name
        $randomNumber = rand(1000, 9999); // Generate a random number
        $randomChar = chr(rand(65, 90)); // Generate a random uppercase ASCII letter (A-Z)
        
        // Concatenate to form the activation link
        $activationLink = "http://" . $companyShort . $randomNumber . $randomChar . ".khibrat";
        
        // Save or do something with the generated link here if needed
        $customer->save();

        // Assuming you have set up a Mailable to handle sending the email
        //Mail::to($customer->email)->send(new CustomerActivated($customer, $activationLink));
