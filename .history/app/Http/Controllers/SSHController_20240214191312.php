    ...
    public function connectAndUpdate(Request $request)
    {
        ...
        foreach ($directories as $directory) {
            ...
            if ($directory === 'back') {
              ...
              return "PGPASSWORD='$pgsqlPassword' psql  -U $pgsqlUsername  -h 127.0.0.1 -p $pgsqlPort  -d \"$dbName\" -f  $sqlFilePath";
              //  $ssh->exec($importCommand);
                ...
                foreach ($files as $file) {
                    ...
                    if (!$sftp->put($remoteFilePath, $localFilePath, SFTP::SOURCE_LOCAL_FILE)) {
                        // Handle error if file transfer fails
                        return response()->json(['error' => "Failed to transfer file: $file"], 500);
                    }
                }
            }
