...
class SSHController extends Controller
{
    ...
    public function connectAndUpdate(Request $request)
    {
        ...
        $sqlFilePath = $customer->request_type === 'demo' ? 
        '/var/www/expertise/back/expertise.sql' : 
        '/var/www/expertise/back/expertiseempty.sql';
        ...
        if ($customer->request_type === 'subscribe') {
            ...
            $this->setupForSubscribe($newClientNumber, $customer->email, $customer->first_name, $randomPassword ,$sqlFilePath);
            ...
            foreach ($this->directories as $directory) {
                $this->setUpClientDirectory($directory, $newClientNumber, $customLink, $localDirectory ,$sqlFilePath);
            }
    ...
    private function setupForSubscribe($newClientNumber, $customerEmail, $firstName, $randomPassword,$sqlFilePath)
    {
        $dbName = $newClientNumber;
    
        $this->createAndImportDatabase($dbName,$sqlFilePath);
    
        $hashedPassword = Hash::make($randomPassword);
    
       $escapedHashForShell = str_replace('$', '\$', $hashedPassword);
        ...
        $insertUserCommand = "PGPASSWORD='{$this->pgsqlPassword}' psql -U '{$this->pgsqlUsername}' -h 127.0.0.1 -p {$this->pgsqlPort} -d \"$dbName\" -c \"INSERT INTO users (email, password, name, id_role) VALUES ('$customerEmail', '$escapedHashForShell', '$escapedFirstName', '27');\"";
  ...
  private function createAndImportDatabase($dbName, $requestType)
{
    $sqlFilePath = ''; // Initialize SQL file path variable
    // Determine the SQL file path based on the request type
    if ($requestType === 'demo') {
        $sqlFilePath = '/var/www/expertise/back/expertise.sql';
    } else if ($requestType === 'subscribe') {
        $sqlFilePath = '/var/www/expertise/back/expertiseempty.sql';
    }
    ...
    if (!empty($sqlFilePath)) {
        $importCommand = "PGPASSWORD='{$this->pgsqlPassword}' psql -U {$this->pgsqlUsername} -h 127.0.0.1 -p {$this->pgsqlPort} -d \"$dbName\" -f $sqlFilePath";
