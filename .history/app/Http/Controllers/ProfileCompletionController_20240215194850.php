...
class ProfileCompletionController extends Controller
{
    public function showCompleteProfileForm($locale, $random_number)
    {
        ...
        return view('details.details', compact('customer', 'stinafTribunals', 'ibtida2iyaTribunals'));
    }
    ...
    public function completeProfile(Request $request)
    {
        ...
        if ($request->has('templates')) {
            ...
            foreach ($files as $file) {
                ...
                $fileName = pathinfo($originalFileName, PATHINFO_FILENAME) . '_' . $customer->id . '.' . $file->getClientOriginalExtension();
                //public_path("storage/templates/$customerId");
                $path = $file->storeAs( ("storage/templates/$customer->id") ,  $fileName);
