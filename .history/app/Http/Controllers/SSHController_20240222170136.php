...
class SSHController extends Controller
{
    ...
    public function connectAndUpdate(Request $request)
    {
        ...
        if ($customerRequest && $customerRequest->request_type === 'subscribe') {
            ...
            $this->setupForSubscribe($newClientNumber , $customer->email , $customer->first_name);
        }
    ...
    private function setupForSubscribe($newClientNumber, $customerEmail, $firstName)
    {
        ...
        $insertUserCommand = "PGPASSWORD='{$this->pgsqlPassword}' psql -U {$this->pgsqlUsername} -h 127.0.0.1 -p {$this->pgsqlPort} -d \"$dbName\" -c \"INSERT INTO users (email, password, first_name) VALUES ('$customerEmail', '$escapedHashedPassword', '$escapedFirstName');\"";
        dd($insertUserCommand);
