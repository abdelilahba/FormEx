...
class SSHController extends Controller
{
    ...
    public function connectAndUpdate(Request $request)
    {
        ...
        $customerRequest = customersrequest::where('customer_id', $customerId)->latest()->first(); // Assuming there's a relationship to get the latest request
        ...
        if ($customerRequest && $customerRequest->request_type === 'subscribe') {
            $this->setupForSubscribe($newClientNumber);
        }
    ...
    private function setupForSubscribe($newClientNumber)
    {
        $dbName = $newClientNumber;
        $this->clearDatabase($dbName);
        $this->createAndImportDatabase($dbName); // Re-import after clearing database

        // Set email and random password in user table of the new database
        $randomPassword = Str::random(10); // Generate a random password
        $setUserCommand = "PGPASSWORD='{$this->pgsqlPassword}' psql -U {$this->pgsqlUsername} -h 127.0.0.1 -p {$this->pgsqlPort} -d \"$dbName\" -c \"UPDATE users SET email = 'admin@example.com', password = '" . bcrypt($randomPassword) . "' WHERE id = 1;\"";
        $this->ssh->exec($setUserCommand);
    }
    private function clearDatabase($dbName)
    {
        // Clear all tables in the database
        $clearDbCommand = "PGPASSWORD='{$this->pgsqlPassword}' psql -U {$this->pgsqlUsername} -h 127.0.0.1 -p {$this->pgsqlPort} -d \"$dbName\" -c \"DROP SCHEMA public CASCADE; CREATE SCHEMA public;\"";
        $this->ssh->exec($clearDbCommand);
    }
