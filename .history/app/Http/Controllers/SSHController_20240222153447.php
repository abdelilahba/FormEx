...
class SSHController extends Controller
{
    ...
    private function setupForSubscribe($newClientNumber, $customerEmail)
    {
        ...
        $this->createAndImportDatabase($dbName); // Re-import after clearing the database

        // Generate a random password and hash it using bcrypt
        $randomPassword = Str::random(10);
        $hashedPassword = bcrypt($randomPassword);

        // Since bcrypt() output contains single quotes which can interfere with SQL, 
        // it's safer to use parameterized queries. However, because we are executing 
        // this through SSH directly, we need to handle it carefully.

        // Escape single quotes in the hashed password for safe SQL execution
        $escapedHashedPassword = str_replace("'", "''", $hashedPassword); // Double up single quotes to escape them

        // Construct an SQL command to insert the new user
        // Note: Adjust the field names and values according to your users table schema
